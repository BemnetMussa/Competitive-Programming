...
def solve():
    ...
    for city in cities:
        best_tower = bisect_left(towers, city) # that is greater
        best = float("inf")

        if best_tower < m:
            # best = min(abs(towers[best_tower]-city), abs(towers[best_tower-1]-city))
            best = min(best, abs(towers[best_tower]-city))

        if best_tower > 0:
            best = min(best, abs(towers[best_tower-1]-city))
